...
<div class="container-fluid p-0">
    ...
    <div class="row w-100 h-100 m-0 p-0">
        ...
        <!-- Header Section -->
        <form action="" method="POST" class="row w-100 p-0 mx-auto align-items-center"
                ...
                <table class="table table-primaria d-none d-lg-table text-center align-center">
                    ...
                    <tbody id="">
                        ...
                        <tr class="d-none d-lg-table-row col-12">
                            ...
                            <td class="col">{{ $item->nombre}} {{ $item->apellido_paterno }} {{ $item->apellido_materno }}</td>
                            <td class="col">
                              {{ $item->tipo_estancia }}
                            </td>
                            @foreach($empresas as $empresa)
                            @if($empresa->idem == $item->idem)
                            <td class="col">{{ $empresa->nombre }}</td>
                            @endif
                            @endforeach
                            <td class="col">
                            @if($item->estatus == 0)
                            ...
                            <td class="align-center col">
                                ...
                                <div class="">
                                    <a href="{{ route('estancias-solicitudes.edit', $item) }}"
                                        class="btn btn-sm btn-primario text-white w-20">
                                        <span>Revisar...</span>
                                    </a>
                                    @if ($item->pdf_solicitud != '')
                                    <a href="{{ asset('archivos/' . $item->pdf_solicitud) }}"
                                        class="btn btn-sm btn-primario text-white w-20">
                                        PDF Solicitud</a>
                                    <a href="{{ asset('archivos/' . $item->pdf_carta) }}"
                                        class="btn btn-sm btn-primario text-white w-20">
                                        PDF Carta</a>
                                    @endif
            ...
            <div class="card-footer">
                <div class="row">
                    <div class="col text-center">
                        <a href=""
                            class="btn btn-sm btn-primario text-white">Revisar...</a>
                    </div>
                </div>
            </div>
        </div>
