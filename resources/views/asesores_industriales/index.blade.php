@extends('layouts.app')
@section('title', 'Asesores Industriales')
...
<div class="container-fluid p-0">
    ...
    <div class="row w-100 h-100 m-0 p-0">
        ...
        <!-- Header Section -->
        <form action="{{ route('asesores-industriales.index') }}" method="POST"
            ...
            <div class="col-12 col-md-4">
                <a href="{{ route('asesores-industriales.create') }}"
                    class="btn btn-primario text-white btn-sm d-block mx-auto w-75">Crear Nuevo</a>
            </div>
        </form>
        <!-- Header Section -->
                ...
                <table class="table table-primaria d-none d-lg-table text-center align-center">
                    ...
                    <tbody id="">
                        ...
                        <tr class="d-none d-lg-table-row col-12">
                            <td class="col-2">{{ $item->rfc }}</td>
                            <td class="col-3">
                                {{ ucfirst(strtolower(substr($item->titulacion, 0, 3))) . '. ' . $item->nombre . ' ' . $item->apellido_paterno . ' ' . $item->apellido_materno }}
                            </td>
                            ...
                            <td class="col-3">
                                <span @class([ 'badge bg-success'=> $item->activo,
                            ...
                            <td class="align-center col-3">
                                ...
                                <div class="row justify-content-evenly w-100">
                                    ...
                                    <a href="{{ route('asesores-industriales.edit', $item) }}"
                                            class="btn btn-sm btn-primario text-white w-20">
                                    ...
                                    <div class="col text-center px-0">
                                    <form action="{{ route('asesores-industriales.toggle.status', $item) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                            <input type="hidden" name="activo" @if($item->activo)
                                            value="0"
                                            @else
                                            value="1"
                                            @endif>
                                    ...
                                    <div class="col text-center px-0">
                                        @if(!$item->activo)
                                        <form action="{{ route('asesores-industriales.destroy', $item) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-danger text-white w-20"
            ...
            <div class="card-footer">
                <div class="row">
                    <div class="col text-center">
                        <a href="{{ route('asesores-industriales.edit', $item) }}"
                            class="btn btn-sm btn-primario text-white">Editar</a>
                    </div>
                    <div class="col text-center">
                        <form action="{{ route('asesores-industriales.toggle.status', $item) }}" method="POST">
                            @csrf
                    ...
                    <div class="col text-center">
                        <form action="{{ route('asesores-industriales.destroy', $item) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <input type="submit" value="Eliminar" class="btn btn-sm btn-secundario text-white"
    ...
    <div class="row my-3 w-100 h-100 p-0">
        <div class="col-12 col-md-3">
            <p class="text-secondary text-center fst-italic">Mostrando {{ $items->count() }} resultados de
                {{ $items->total() }}</p>
        </div>
